(function () {
    ...
    chunks.editing = function (host) {
        ...
        const dbeSelector = host.selector;
        ...
        const dbeRestoreOwnedAttributes = host.restoreOwnedAttributes;
        ...
        const dbeIsMac = host.platform.isMac;
        const clearMultiSel = host.multiSelection.clear;
        const navRowById = host.navigator.rowById;
        const navSelect = host.navigator.select;
        const contextTarget = host.context.getTarget;
        const driveContextMenuItem = host.commands.driveContextMenuItem;
        const expandSubtree = host.commands.expandNavigatorSubtree;
        const dbePresenceDirtyChanged = host.presenceDirtyChanged;
    ...
    function wrap(type, idsOpt) {
        ...
        const sf = store();
        const mods = sf.storeGet('modules');
        if (!mods) { return; }
        const ids = (idsOpt && idsOpt.length ? idsOpt : [contextTarget() || sf.storeGet('activeModule')])
            .filter((id) => { return !!mods[id]; });
        if (!ids.length) { return; }

        // Only siblings can share one new parent; the menu disables Wrap for a
        // mixed selection, this is the backstop.
        const parentId = mods[ids[0]].parent || '';
        if (ids.some((id) => { return (mods[id].parent || '') !== parentId; })) {
        ...
        const idx = sf.storeGet('indexes') || {};
        const siblings = idx[parentId || 'root'] ? [].concat(idx[parentId || 'root']) : [];
        let order = siblings.filter((id) => { return ids.indexOf(id) !== -1; });
        if (!order.length) { order = ids.slice(); }
        let firstAt = siblings.indexOf(order[0]);
        if (firstAt < 0) { firstAt = siblings.length; }

        const makeId = dbeMakeId;
        const newId = makeId();
        let attachId = newId; // where the selected elements are moved into
        ...
        order.forEach((id, i) => { storeMoveModule(sf, id, attachId, i); });
        ...
        if (type === 'template') {
            ...
            const typeLabel = type === 'collection' ? dbeT('wrapTypeCollection', 'collection + template') : type;
            undoToast(dbeFmt(dbeTn(order.length,
    ...
    function storeAddModule(sf, module, parentId, index) {
        ...
        sf.storeSet('addModule', { module });
        sf.storeSet('addIndex', { module });
        if (typeof index === 'number') {
            const key = parentId || 'root';
            const idx = sf.storeGet('indexes') || {};
            const arr = idx[key] ? [].concat(idx[key]) : [];
            const at = arr.indexOf(module.id);
            if (at !== -1 && at !== index) {
                arr.splice(at, 1);
                arr.splice(index, 0, module.id);
                const next = Object.assign({}, idx);
                next[key] = arr;
    ...
    function storeMoveModule(sf, moduleId, newParentId, newIndex) {
        ...
        const idx = sf.storeGet('indexes') || {};
        const oldParent = mods[moduleId].parent || 'root';
        const newParent = (typeof newParentId === 'string') ? (newParentId || 'root') : oldParent;
        const oldIndex = (idx[oldParent] ? [].concat(idx[oldParent]) : []).indexOf(moduleId);
        sf.storeSet('moveModule', {
            oldParent, sourceId: moduleId, oldIndex,
            newIndex: (typeof newIndex === 'number') ? newIndex : 0, newParent
        });
    }
    ...
    function dbeMakeId(existing) {
        const used = existing || modules() || {};
        let id;
        do {
            id = 'u' + Array.from({ length: 9 }, () => {
    ...
    const DBE_URL_ATTRS = { href: 1, src: 1, action: 1, formaction: 1, poster: 1, 'xlink:href': 1 };
    ...
    function dbeDecodeEntities(value) {
        ...
        try {
            ...
            const parsed = new DOMParser().parseFromString('<i data-dbe-value="' + escaped + '"></i>', 'text/html');
            const holder = parsed.body && parsed.body.firstElementChild;
    ...
    function dbeDangerousUrl(value) {
        ...
        const v = dbeDecodeEntities(value).replace(/[\u0000-\u0020]+/g, '').toLowerCase();
    ...
    function dbeElementModule(tag, opts) {
        opts = opts || {};
        const settings = [{ name: 'tag', value: tag }];
        ...
        (opts.attrs || []).forEach((a) => {
            ...
            const value = dbeDecodeEntities(a.value);
        ...
        return { id: dbeMakeId(), name: 'HtmlElement', label, settings };
    }
    ...
    function dbeEmmetParse(str) {
        ...
        function parseElement() {
            const node = { tag: '', subTag: null, id: '', classes: [], attrs: [], text: null, count: 1, children: [] };
            ...
            const tm = /^([A-Za-z][A-Za-z0-9]*)(?::([A-Za-z][A-Za-z0-9]*))?/.exec(s.slice(i));
            if (tm && tm[0]) {
                node.tag = tm[1];
            ...
            while (i < s.length) {
                const c = s[i];
                if (c === '#') {
                    i++; const im = /^[A-Za-z0-9_-]+/.exec(s.slice(i)); if (!im) { throw 0; }
                    node.id = im[0]; i += im[0].length;
                } else if (c === '.') {
                    i++; const cm = /^[A-Za-z0-9_-]+/.exec(s.slice(i)); if (!cm) { throw 0; }
                    node.classes.push(cm[0]); i += cm[0].length;
                } else if (c === '[') {
                    const abEnd = s.indexOf(']', i); if (abEnd < 0) { throw 0; }
                    const inner = s.slice(i + 1, abEnd);
                    const re = /([A-Za-z_:][-A-Za-z0-9_:.]*)(?:=("([^"]*)"|'([^']*)'|[^\s\]]+))?/g;
                    let am;
                    while ((am = re.exec(inner))) {
                        const av = am[3] != null ? am[3] : (am[4] != null ? am[4] : (am[2] != null ? am[2] : ''));
                        node.attrs.push({ name: am[1], value: av });
                    }
                    i = abEnd + 1;
                } else if (c === '{') {
                    const end = s.indexOf('}', i); if (end < 0) { throw 0; }
                    node.text = s.slice(i + 1, end); i = end + 1;
                } else { break; }
            }
            if (!node.tag && !node.classes.length && !node.id && node.text == null && !node.attrs.length) { throw 0; }
            if (s[i] === '*') {
                i++; const nm = /^[0-9]+/.exec(s.slice(i)); if (!nm) { throw 0; }
                node.count = Math.max(1, Math.min(50, parseInt(nm[0], 10))); i += nm[0].length;
            }
            return node;
        }
        const roots = [];
        let cur = parseElement();
        roots.push(cur);
        let level = roots; // the array `cur` currently lives in (its sibling list)
        while (i < s.length) {
            const op = s[i];
            if (op === '>') { i++; level = cur.children; const ch = parseElement(); level.push(ch); cur = ch; }
            else if (op === '+') { i++; const sib = parseElement(); level.push(sib); cur = sib; }
            else { throw 0; }
        }
        return roots;
    }
    ...
    /* Reserved Emmet words: `collection`, `subcollection` and `template`
       ...
       inline. */
    const DBE_EMMET_WORDS = { collection: 'Collection', subcollection: 'SubCollection', template: 'Template' };

    function dbeEmmetNodeToModule(node) {
        const word = DBE_EMMET_WORDS[(node.tag || '').toLowerCase()];
        if (word) {
            const settings = [];
            if (word !== 'Template') {
            ...
            if (attrList.length) { settings.push({ name: 'htmlAttribute', value: attrList }); }
            return { id: dbeMakeId(), name: word, label: word, settings };
        }
        const tag = node.tag || ((node.text != null && !node.classes.length && !node.id) ? 'span' : 'div');
    ...
    function dbeEmmetStructureError(targetId, roots) {
        ...
        (function walk(list) {
            list.forEach((n) => {
                if (err) { return; }
                const w = DBE_EMMET_WORDS[(n.tag || '').toLowerCase()];
    ...
    function dbeEmmetInsert(targetId, roots) {
        ...
        const VOID = /^(img|input|br|hr|area|base|col|embed|link|meta|param|source|track|wbr)$/i;
        const tag = ((mods[targetId].settings || []).filter((x) => { return x.name === 'tag'; })[0] || {}).value || '';
        let parentId, startIndex;
        const indexes = sf.storeGet('indexes') || {};
        if (!VOID.test(tag)) {
            parentId = targetId;
            startIndex = (indexes[targetId] || []).length;
        } else {
            parentId = mods[targetId].parent || '';
            const sibs = [].concat(indexes[parentId || 'root'] || []);
        ...
        (function buildInto(nodeList, pId, startIdx) {
            ...
            nodeList.forEach((node) => {
                for (let rep = 0; rep < (node.count || 1); rep++) {
                    const mod = dbeEmmetNodeToModule(node);
    ...
    function dbeInsertSibling(targetId, dir, module) {
        ...
        const parent = mods[targetId].parent || '';
        const idx = sf.storeGet('indexes') || {};
        const sibs = idx[parent || 'root'] ? [].concat(idx[parent || 'root']) : [];
        let at = sibs.indexOf(targetId);
    ...
    function dbeInsertSection(targetId, dir) {
        const sf = store();
        const newId = dbeInsertSibling(targetId, dir, dbeElementModule('section'));
    ...
    function dbeUpdateModuleSettings(id, mutate, historyLabel, replacementLabel) {
        ...
        const updated = JSON.parse(JSON.stringify(mods[id]));
        const previousSettings = JSON.parse(JSON.stringify(updated.settings || []));
    ...
    function dbeReselectToRehydrate(id) {
        const backRow = document.querySelector('.uniRightPanel .uni-tree-node-' + id);
        if (!backRow) { return; }
        const mods = modules() || {};
        const other = (mods[id] && mods[id].parent) || '';
        let otherRow = other && document.querySelector('.uniRightPanel .uni-tree-node-' + other);
        if (!otherRow) {
            otherRow = [].slice.call(document.querySelectorAll('.uniRightPanel .uniModTree__list button.uniModTree__item'))
                .filter((r) => { return r !== backRow; })[0];
        }
        if (!otherRow) { return; }
        ...
        const tabLabel = (([].slice.call(document.querySelectorAll('.uniLeftPanel .uniPanelTabs__tab'))
            .filter((t) => { return t.classList.contains('active'); })[0] || {}).textContent || '').trim();
        clickSeq(otherRow);
        waitFor(() => { return (activeId() && activeId() !== id) ? true : null; }, () => {
            const again = document.querySelector('.uniRightPanel .uni-tree-node-' + id);
            if (again) { clickSeq(again); }
            if (!tabLabel) { return; }
            waitFor(() => { return activeId() === id ? true : null; }, () => {
                const tab = [].slice.call(document.querySelectorAll('.uniLeftPanel .uniPanelTabs__tab'))
                    .filter((t) => { return (t.textContent || '').trim() === tabLabel; })[0];
    ...
    function dbeAddClasses(id, classes) {
        return dbeUpdateModuleSettings(id, (settings) => {
            let tc = settings.filter((s) => { return s.name === 'tagClass'; })[0];
            if (!tc) { tc = { name: 'tagClass', value: [] }; settings.push(tc); }
            if (!Array.isArray(tc.value)) { tc.value = []; }
            classes.forEach((c) => { if (tc.value.indexOf(c) === -1) { tc.value.push(c); } });
    ...
    const DBE_IMG_PLACEHOLDER = 'data:image/svg+xml,' + encodeURIComponent(
    ...
    function dbeApplyImageDefaults(id) {
        ...
        const tag = ((m.settings || []).filter((s) => { return s.name === 'tag'; })[0] || {}).value;
        if (tag !== 'img') { return; }
        ...
        const attrs = ((m.settings || []).filter((s) => { return s.name === 'htmlAttribute'; })[0] || {}).value || [];
        const hasSrc = attrs.some((a) => { return a && a.name === 'src' && a.value; });
        const hasAlt = attrs.some((a) => { return a && a.name === 'alt' && a.value != null; });
        if (hasSrc && hasAlt) { return; }
        const wasBusy = dbeUndoBusy;
        dbeUndoBusy = true;
        try {
            dbeUpdateModuleSettings(id, (settings) => {
                let attr = settings.filter((s) => { return s.name === 'htmlAttribute'; })[0];
                ...
                if (!hasAlt) {
                    const alt = attr.value.filter((a) => { return a && a.name === 'alt'; })[0];
    ...
    function hookImageDefaults() {
        dbeBindOwnedHook(DBE_EDITING_OWNER, 'builderius.Module.added', 'dbeImageDefaults', (p) => {
            ...
            dbeSetOwnedTimeout(DBE_EDITING_OWNER, () => {
    ...
    function dbeMoveLocation(id) {
        ...
        const mod = mods[id];
        if (!mod) { return null; }
        const parentId = mod.parent || '';
        const idx = sf.storeGet('indexes') || {};
        const siblings = [].concat(idx[parentId || 'root'] || []);
        return {
            sf,
            mods,
            mod,
    ...
    function dbeModuleCanContainChildren(mod) {
        ...
        const tag = String(dbeSettingVal(mod, 'tag') || 'div').toLowerCase();
        return !DBE_HTML_VOID[tag];
    }

    function dbeFocusMovedRow(id) {
        waitFor(() => { return navRowById(id); }, (row) => {
    ...
    function dbeIndentTarget(id) {
        ...
        const targetId = loc.siblings[loc.index - 1];
        return dbeModuleCanContainChildren(loc.mods[targetId]) ? targetId : null;
    }

    function indentElement(id) {
        const targetId = dbeIndentTarget(id);
        if (!targetId) { return false; }
        const indexes = store().storeGet('indexes') || {};
    ...
    function outdentElement(id) {
        ...
        const parent = loc.mods[loc.parentId];
        if (!parent) { return false; }
        const grandParentId = parent.parent || '';
        const indexes = loc.sf.storeGet('indexes') || {};
        const parentSiblings = [].concat(indexes[grandParentId || 'root'] || []);
        const parentIndex = parentSiblings.indexOf(loc.parentId);
    ...
    function selectParentOf(id) {
        const mods = modules() || {};
        const p = mods[id] && mods[id].parent;
        if (!p) { return; }
        const row = document.querySelector('.uniRightPanel .uni-tree-node-' + p);
        if (row) { clickSeq(row); }
    ...
    function unwrap(id) {
        ...
        const mods = sf.storeGet('modules') || {};
        if (!mods[id]) { return; }
        const parent = mods[id].parent || '';
        const idx = sf.storeGet('indexes') || {};
        const parentSibs = idx[parent || 'root'] ? [].concat(idx[parent || 'root']) : [];
        let wrapperAt = parentSibs.indexOf(id);
        if (wrapperAt < 0) { wrapperAt = parentSibs.length; }
        const kids = idx[id] ? [].concat(idx[id]) : [];
        if (!kids.length) { undoToast(dbeT('nothingToUnwrap', 'Nothing to unwrap: this element has no children')); return; }
        kids.forEach((kid, i) => { storeMoveModule(sf, kid, parent, wrapperAt + i); });
        ...
        setTimeout(() => { driveContextMenuItem(id, 'Remove', () => {}); }, 200);
    ...
    function defaultLabelFor(id) {
        const mods = modules();
        const mod = mods && mods[id];
        if (!mod) { return ''; }
        const tag = (mod.settings || []).filter((s) => { return s.name === 'tag'; })[0];
    ...
    function dbeNavigatorLabel(row) {
        const holder = row && row.querySelector('span');
        if (!holder) { return ''; }
        const spoken = holder.querySelector('.dbe-visually-hidden');
        const raw = ((spoken || holder).textContent || '').trim();
        const classAt = raw.indexOf(' .');
    ...
    function dbeFinishCancelledRename(st, restoreFocus) {
        dbeSetOwnedTimeout(DBE_EDITING_OWNER, () => {
            ...
            const row = document.querySelector('.uniRightPanel .uni-tree-node-' + st.id);
            const current = dbeNavigatorLabel(row);
    ...
    function startRename(id) {
        ...
        const wrapper = row.closest('.uniModTree__itemContentWrapper') || row.parentElement;
        const li = row.closest('li.uniModTree__itemDrag');
        const oldLabel = dbeNavigatorLabel(row) || mods[id].label || '';
        ...
        ['keyup', 'keypress', 'pointerdown', 'mousedown', 'pointerup', 'mouseup', 'click', 'dblclick']
            .forEach((t) => { input.addEventListener(t, (ev) => { ev.stopPropagation(); }); });
        input.addEventListener('blur', () => { closeRename(true); });
    ...
    function moduleClasses(mod) {
        const s = (mod && mod.settings || []).filter((x) => { return x.name === 'tagClass'; })[0];
    ...
    function bemModuleTag(mod, canvasTag) {
        const t = (mod && mod.settings || []).filter((s) => { return s.name === 'tag'; })[0];
    ...
    function bemCollectRows(rootId) {
        ...
        const kids = dbeChildIndex(mods);
        const rows = [];
        (function walk(id, depth) {
            const mod = mods[id];
            if (!mod) { return; }
            const canvasEl = idoc && idoc.querySelector('.uni-node-' + id);
            const tag = bemModuleTag(mod, canvasEl ? canvasEl.tagName.toLowerCase() : null);
            rows.push({
                id, depth, mod, tag,
    ...
    function bemDescriptor(row) {
        const label = (row.mod.label || '').trim();
    ...
    function bemSuggest(block, rows) {
        ...
        return rows.map((row, i) => {
            ...
            let name = i === 0 ? block : block + '__' + bemDescriptor(row);
            const base = name;
            let n = 2;
    ...
    const BEM_CLASS_RE = /^-?[A-Za-z_][A-Za-z0-9_-]*$/;

    function dbeRemovePriorBemDialog() {
        const old = document.querySelector('dialog.dbe-bem');
        if (!old) { return; }
    ...
    function openAutoBemDialog(rootId) {
        ...
        const rows = bemCollectRows(rootId);
        ...
        let applyBtn = null; // forward ref for the count refresh

        const suggestions = bemSuggest(blockInput.value, rows);
        const rowUi = rows.map((row, i) => {
            ...
            field.addEventListener('input', () => { field.dataset.dbeEdited = '1'; });
        ...
        blockInput.addEventListener('input', () => {
            const next = bemSuggest(slugify(blockInput.value) || 'block', rows);
            rowUi.forEach((ui, i) => {
        ...
        applyBtn.addEventListener('click', () => {
            const jobs = pendingRows().map((ui) => {
                return { id: ui.row.id, className: (ui.field.value || '').trim() };
            });
            const bad = jobs.filter((j) => { return !BEM_CLASS_RE.test(j.className); });
            if (bad.length) {
        ...
        ['pointerdown', 'mousedown', 'click'].forEach((t) => {
            dlg.addEventListener(t, (e) => { e.stopPropagation(); });
        });
    ...
    function bemClosePicker() {
        const input = document.querySelector('.uniLeftPanel .uniSystemSelectClasses__search');
        if (input) {
    ...
    function bemApplyOne(id, className, done) {
        ...
        (function selectRow() {
            const row = document.querySelector('.uniRightPanel .uni-tree-node-' + id);
            if (row) { clickSeq(row); }
            waitFor(() => { return activeId() === id || null; }, (ok) => {
                if (!ok) {
                ...
                waitFor(() => {
                    ...
                    const opener = picker.querySelector('.uniSystemSelectClasses__fakeInput') ||
                    ...
                    waitFor(() => {
                        ...
                        setTimeout(() => {
                            ...
                            }, (landed) => { bemClosePicker(); done(!!landed); }, 40);
                        }, 80);
    ...
    function applyBemQueue(jobs) {
        ...
        dbeBemBusy = true;
        const progress = bemProgressCover();
        const total = jobs.length;
        let added = 0;
        const retry = [];   // stragglers to re-attempt once the panel is warm
        ...
        function runPass(list, isRetry, whenDone) {
            ...
            (function next() {
                ...
                bemApplyOne(job.id, job.className, (ok) => {
        ...
        setTimeout(() => {
            runPass(jobs, false, () => {
                if (retry.length && !progress.aborted()) {
                    setTimeout(() => { runPass(retry, true, finish); }, 250);
                } else { finish(); }
            });
        }, 350);
    }
    ...
    let undoStack = [];
    let redoStack = [];
    let dbeUndoBusy = false;
    let dbeHistoryRestoredIds = {};
    let toastTimer = null;
    let toastDuration = 2600;

    function dbeHistoryActionAvailable(action) {
        const stack = action === 'redo' ? redoStack : undoStack;
    ...
    function dbeHideToast(t) {
        ...
        const action = t.querySelector('.dbe-undo-toast__action');
        if (action) { action.tabIndex = -1; }
    }

    function dbeScheduleToast(t) {
        dbeClearOwnedTimeout(DBE_EDITING_OWNER, toastTimer);
        toastTimer = dbeSetOwnedTimeout(DBE_EDITING_OWNER, () => { dbeHideToast(t); }, toastDuration);
    }

    function undoToast(msg, action) {
        let t = document.querySelector('.dbe-undo-toast');
        if (!t) {
            t = document.createElement('div');
            t.className = 'dbe-undo-toast';
            t.addEventListener('mouseenter', () => { dbeClearOwnedTimeout(DBE_EDITING_OWNER, toastTimer); });
            t.addEventListener('mouseleave', () => { dbeScheduleToast(t); });
            t.addEventListener('focusin', () => { dbeClearOwnedTimeout(DBE_EDITING_OWNER, toastTimer); });
        ...
        const hasAction = (action === 'undo' || action === 'redo') && dbeHistoryActionAvailable(action);
        if (hasAction) {
    ...
    function dbeHistoryPlacement(mods, indexes, id) {
        const mod = mods && mods[id];
        const parentId = (mod && mod.parent) || '';
        const siblings = [].concat((indexes && indexes[parentId || 'root']) || []);
    ...
    function dbeHistoryLiveId(id, mods) {
        ...
        while (current && limit-- > 0) {
            ...
            const next = dbeHistoryRestoredIds[current];
    ...
    function dbeRestoreTargetIndex(rec, siblings, mods, newId) {
        const afterId = dbeHistoryLiveId(rec.afterId, mods);
        const afterAt = afterId && afterId !== newId ? siblings.indexOf(afterId) : -1;
        if (afterAt >= 0) { return afterAt; }
        const beforeId = dbeHistoryLiveId(rec.beforeId, mods);
        const beforeAt = beforeId && beforeId !== newId ? siblings.indexOf(beforeId) : -1;
    ...
    function dbePositionRestoredModule(newId, rec) {
        ...
        const location = dbeMoveLocation(newId);
        if (!location || location.index < 0) { return; }
        const target = dbeRestoreTargetIndex(rec, location.siblings, location.mods, newId);
    ...
    function hookHistoryCapture() {
        ...
        dbeBindOwnedHook(DBE_EDITING_OWNER, 'builderius.Module.deleted', 'dbeUndoCaptureDel', (p) => {
            if (dbeUndoBusy || !p || !p.id) { return; }
            let hist;
            try { hist = store().storeGet('history') || []; } catch (e) { return; }
            let snap = null;
            for (let i = hist.length - 1; i >= 0; i--) {
                const candidate = hist[i].snapshot;
                const sm = candidate && candidate.modules;
                if (sm && sm[p.id]) { snap = candidate; break; }
            }
            if (!snap) { return; }
            const snapMods = snap.modules;
            const placement = dbeHistoryPlacement(snapMods, snap.indexes || {}, p.id);
            dbeHistoryPush({
                op: 'restore',
                id: p.id,
        ...
        dbeBindOwnedHook(DBE_EDITING_OWNER, 'builderius.Module.added', 'dbeUndoCaptureAdd', (p) => {
            if (dbeUndoBusy || !p || !p.id) { return; }
            const m = (modules() || {})[p.id];
            dbeHistoryPush({
                op: 'remove',
                id: p.id,
    ...
    function dbeHtmlAutoCloseTag(value, start, end) {
        ...
        let openAt = -1;
        let scanningQuote = '';
        for (let scanAt = 0; scanAt < before.length; scanAt++) {
            const scanChar = before.charAt(scanAt);
            if (openAt < 0) {
        ...
        const inside = before.slice(openAt + 1);
        if (!inside || /^[\s]*[!/?]/.test(inside) || /\/\s*$/.test(inside)) { return ''; }

        let quote = '';
        for (let i = 0; i < inside.length; i++) {
            const ch = inside.charAt(i);
            if (!quote && (ch === '"' || ch === "'")) {
                quote = ch;
            } else if (quote === ch) {
                quote = '';
        ...
        const match = inside.match(/^\s*([A-Za-z][A-Za-z0-9:-]*)(?:\s|$)/);
        if (!match || DBE_HTML_VOID[match[1].toLowerCase()]) { return ''; }

        const nextClosing = after.match(/^\s*<\/\s*([A-Za-z][A-Za-z0-9:-]*)\s*>/);
    ...
    function dbeHtmlClassItems(editorValue) {
        const items = {};
        function item(name) {
            if (!items[name]) { items[name] = { name, modules: 0, markup: false, rules: [] }; }
            return items[name];
        }
        const currentModules = modules() || {};
        Object.keys(currentModules).forEach((id) => {
            moduleClasses(currentModules[id]).forEach((name) => {
                if (typeof name === 'string' && name) { item(name).modules += 1; }
            });
        });
        String(editorValue || '').replace(/\bclass(?:Name)?\s*=\s*["']([^"']*)["']/gi, (all, value) => {
        ...
        const previewDocument = frame && frame.contentDocument;
        function collectFromRules(rules) {
            for (let i = 0; rules && i < rules.length; i++) {
                const rule = rules[i];
                if (rule.selectorText) {
                    const matches = rule.selectorText.matchAll(/\.(-?[_a-zA-Z]+[_a-zA-Z0-9-]*)/g);
                    for (const match of matches) {
                        const found = item(match[1]);
                        const summary = rule.selectorText + ' { ' + String(rule.style && rule.style.cssText || '') + ' }';
        ...
        if (previewDocument) {
            const sheets = [].slice.call(previewDocument.styleSheets || [])
        ...
        }).sort().map((name) => { return items[name]; });
    }
    ...
    function dbeHtmlCompletionContext(model, position) {
        ...
        const dynamic = before.match(/(\[\[|\{\{)([A-Za-z0-9_.-]*)$/);
        if (dynamic) {
        ...
        const tagMatch = inside.match(/^\s*([A-Za-z][A-Za-z0-9:-]*)?/);
        const tag = tagMatch && tagMatch[1] ? tagMatch[1].toLowerCase() : '';
        if (!/\s/.test(inside) && inside.indexOf('=') === -1) {
            return { type: 'tag', tag, partial: tag, start: offset - tag.length };
        }
        const valueMatch = inside.match(/([A-Za-z_:][A-Za-z0-9_.:-]*)\s*=\s*(["'])([^"']*)$/);
        if (valueMatch) {
            const valuePartial = valueMatch[3].split(/\s+/).pop() || '';
            return {
        ...
        const attrMatch = inside.match(/(?:^|\s)([A-Za-z_:][A-Za-z0-9_.:-]*)?$/);
        if (!attrMatch) { return null; }
        const partial = attrMatch[1] || '';
        return { type: 'attribute', tag, partial, start: offset - partial.length, inside };
    }
    ...
    function dbeHtmlAnalyse(value) {
        ...
        while ((at = value.indexOf('<', at)) !== -1) {
            if (value.slice(at, at + 4) === '<!--') {
                const commentEnd = value.indexOf('-->', at + 4);
                if (commentEnd === -1) {
                    return { tokens, pairs: {}, error: {
                        offset: at, length: 4,
            ...
            const lead = value.slice(at + 1).match(/^\s*(\/)?\s*([A-Za-z][A-Za-z0-9:-]*)/);
            if (!lead) { at += 1; continue; }
            let quote = '';
            let end = at + 1;
            for (; end < value.length; end++) {
                const ch = value.charAt(end);
                if (quote) {
                    if (ch === quote) { quote = ''; }
                } else if (ch === '"' || ch === "'") {
                    quote = ch;
            ...
            const nameOffset = raw.indexOf(lead[2]);
            tokens.push({
        ...
        tokens.forEach((token, index) => {
            ...
            for (let between = stack.length - 1; between > matchAt; between--) {
        ...
        const bad = tokens.filter((token) => { return !!token.error; })[0];
        if (bad) {
            return { tokens, pairs, error: {
                offset: bad.nameStart, length: bad.nameEnd - bad.nameStart, message: bad.error
            } };
        }
        for (let u = stack.length - 1; u >= 0; u--) {
        ...
        return { tokens, pairs, error: null };
    }
    ...
    function dbeFormatHtml(value) {
        ...
        const lines = [];
        function attrs(el) {
            return [].slice.call(el.attributes || []).map((a) => {
        ...
        function write(node, depth) {
            ...
            const contentNodes = tag === 'template' && node.content
                ? [].slice.call(node.content.childNodes)
                : [].slice.call(node.childNodes);
            const elements = contentNodes.filter((child) => { return child.nodeType === 1; });
            const text = contentNodes.filter((child) => { return child.nodeType === 3; })
                .map((child) => { return child.textContent; }).join('');
            if (!elements.length || text.trim()) {
                lines.push(pad + open + node.innerHTML + '</' + tag + '>');
                return;
            }
            lines.push(pad + open);
            contentNodes.forEach((child) => { write(child, depth + 1); });
            lines.push(pad + '</' + tag + '>');
        }
        [].slice.call(template.content.childNodes).forEach((node) => { write(node, 0); });
        return lines.join('\n');
    }

    function dbeSettingVal(mod, name) {
        const s = ((mod && mod.settings) || []).filter((x) => { return x.name === name; })[0];
    ...
    /* The module types the HTML dialogs can express. Collections and
       ...
       composites. */
    const DBE_HTML_MODULES = { HtmlElement: 1, Collection: 1, SubCollection: 1, Template: 1, SvgCode: 1, Component: 1 };
    ...
    function dbeComponentRegistry() {
        ...
        const data = store().storeGet('componentsData') || {};
        list.forEach((c) => {
            if (!c || !c.name) { return; }
            const props = {};
            const cfg = data[c.name];
            ((cfg && cfg.settings) || []).forEach((s) => {
            ...
            reg[c.name] = { label: c.title || c.name, props };
        });
        return reg;
    }

    function dbeHtmlCompletionItems(api, model, position, context, classItems) {
        const kinds = api.languages.CompletionItemKind || {};
        const rules = api.languages.CompletionItemInsertTextRule || {};
        const range = dbeHtmlCompletionRange(model, position, context);
        const suggestions = [];
        ...
        const registry = dbeComponentRegistry();
        const componentMatch = (context.inside || '').match(/\bname\s*=\s*["']([^"']+)["']/i);
        const component = componentMatch && registry[componentMatch[1]];
        if (context.type === 'attribute') {
            ...
            ].forEach((def) => { add(def[0], def[1], def[2], kinds.Property || 9, '', true); });
            if (context.tag === 'dbe-component') {
                ...
                if (component) {
                    Object.keys(component.props).forEach((name) => {
                        const def = component.props[name];
        ...
        if (component && component.props[context.attribute]) {
            const prop = component.props[context.attribute];
            ...
            let options = prop.options || prop.choices || prop.values || [];
            if (!Array.isArray(options) && typeof options === 'object') { options = Object.keys(options); }
            options.forEach((option) => {
                const value = typeof option === 'object' ? (option.value || option.name || option.label) : option;
    ...
    function dbeSerializeLabel(m, defaultLabel) {
        const label = String((m && m.label) || '').replace(/\s+/g, ' ').trim();
    ...
    function dbeSerializeSubtree(rootId) {
        const mods = modules() || {};
        const idx = store().storeGet('indexes') || {};
        function ser(id, depth) {
            ...
            if (m.name === 'SvgCode') {
                let svg = String(dbeSettingVal(m, 'contentSvg') || '<svg xmlns="http://www.w3.org/2000/svg"></svg>');
                svg = svg.replace(/<svg\b/i, '<svg data-dbe-id="' + id + '"');
                return svg.split('\n').map((l) => { return pad + l; }).join('\n');
            }
            ...
            if (m.name === 'Component') {
                const slug = String(dbeSettingVal(m, 'componentName') || '');
                let copen = '<dbe-component name="' + dbeHtmlEscapeAttr(slug) + '"';
                (dbeSettingVal(m, 'componentProperties') || []).forEach((p) => {
                    if (!p || !p.name) { return; }
                    copen += ' ' + p.name + '="' + dbeHtmlEscapeAttr(p.value == null ? '' : p.value) + '"';
                });
                const creg = dbeComponentRegistry();
                let componentLabel = String(m.label || '').replace(/\s+/g, ' ').trim();
                if (!componentLabel) {
            ...
            const content = dbeSettingVal(m, 'content');
            ...
            kids.forEach((k) => { lines.push(ser(k, depth + 1)); });
            lines.push(pad + '</' + tag + '>');
            return lines.join('\n');
        }
        return ser(rootId, 0);
    ...
    const DBE_HTML_LIMITS = { bytes: 262144, nodes: 5000, depth: 100 };

    function dbeParseHtmlFragment(html, origIds) {
        const byteLength = new Blob([String(html)]).size;
        ...
        const allNodes = doc.body.querySelectorAll('*').length;
        ...
        const depthStack = [].slice.call(doc.body.children).map((el) => { return { el, depth: 1 }; });
        while (depthStack.length) {
            const depthItem = depthStack.pop();
            ...
            const depthHost = (depthItem.el.tagName.toLowerCase() === 'template' && depthItem.el.content) ? depthItem.el.content : depthItem.el;
            [].slice.call(depthHost.children).forEach((child) => {
        ...
        const registry = dbeComponentRegistry();
        const STRIP_TAGS = { script: 1, style: 1, link: 1, meta: 1, iframe: 1, object: 1, embed: 1, noscript: 1, base: 1, math: 1 };
        function claim(marker, moduleName, representation) {
            ...
            const expected = origIds[marker];
            const valid = representation === 'keep' ? !DBE_HTML_MODULES[expected] : expected === moduleName;
            if (!valid) {
                throw dbeFmt(
        ...
        function convertSvg(el) {
            const node = { existingId: null, module: 'SvgCode', tag: 'svg', tagId: '', classes: [], attrs: [], content: '', children: [], svg: '', label: '' };
            const marker = el.getAttribute('data-dbe-id');
            ...
            const svgLabel = el.getAttribute('data-dbe-label');
            ...
            const SVG_DROP = { script: 1, foreignobject: 1, style: 1, handler: 1, listener: 1 };
            const SVG_ANIM = { animate: 1, set: 1, animatetransform: 1, animatemotion: 1 };
            let ariaNoted = false;
            [el].concat([].slice.call(el.querySelectorAll('*'))).forEach((d) => {
                if (d !== el && !el.contains(d)) { return; } // removed with an ancestor already
                const ln = (d.localName || d.tagName || '').toLowerCase();
                ...
                if (ln === 'use') {
                    const uref = (d.getAttribute('href') || d.getAttribute('xlink:href') || '').trim();
                ...
                if (SVG_ANIM[ln]) {
                    const target = (d.getAttribute('attributeName') || '').toLowerCase();
                    if (target === 'href' || target === 'xlink:href') {
                        const vals = [d.getAttribute('to'), d.getAttribute('from'), d.getAttribute('by')]
                            .concat((d.getAttribute('values') || '').split(';'));
                        if (vals.some((x) => { return x && dbeDangerousUrl(x); })) {
                            stripped.push('<' + ln + '>'); d.parentNode.removeChild(d); return;
                        }
                    }
                }
                [].slice.call(d.attributes).forEach((a) => {
            ...
            const svgLines = el.outerHTML.split('\n');
            if (svgLines.length > 1) {
                const indents = svgLines.slice(1).filter((l) => { return l.trim(); })
                    .map((l) => { return /^\s*/.exec(l)[0].length; });
                const minIndent = indents.length ? Math.min.apply(null, indents) : 0;
                node.svg = [svgLines[0]].concat(svgLines.slice(1).map((l) => {
        ...
        function convert(el) {
            ...
            if (tag === 'dbe-keep') {
                const kMarker = el.getAttribute('data-dbe-id');
            ...
            if (tag === 'dbe-component') {
                const cslug = String(el.getAttribute('name') || '').trim();
                if (!cslug || !registry[cslug]) {
                    const invalidComponentMarker = String(el.getAttribute('data-dbe-id') || '').trim();
                    ...
                    const avail = Object.keys(registry).join(', ');
                    stripped.push('<dbe-component name="' + cslug + '"> (' + (avail ? 'unknown component; available: ' + avail : 'no components registered') + ')');
                    return null;
                }
                const cnode = { existingId: null, module: 'Component', componentName: cslug, props: [], children: [], label: '' };
                const declared = registry[cslug].props;
                let cMarker = '';
            ...
            const node = { existingId: null, module: 'HtmlElement', tag, tagId: '', classes: [], attrs: [], content: '', children: [], label: '' };
            ...
            [].slice.call(el.attributes).forEach((a) => {
                ...
                // reach it here.
                const blocked = dbeAttrBlocked(n, v);
            ...
            const kidsHost = (tag === 'template' && el.content) ? el.content : el;
            [].slice.call(kidsHost.childNodes).forEach((ch) => {
                if (ch.nodeType === 3) {
                    const t = dbeEscapeRawText(ch.textContent.replace(/\s+/g, ' ').trim());
                    if (!t) { return; }
        ...
        const roots = [].slice.call(doc.body.children).map(convert).filter(Boolean);
        return { roots, stripped, unknownMarkers: Object.keys(unknownMarkers) };
    }

    /* The Edit-as-HTML shape: one root, or a structural error. */
    function dbeParseHtmlTree(html, origIds, rootId) {
        const parsed = dbeParseHtmlFragment(html, origIds);
    ...
    function dbeNodeSettings(node) {
        const moduleName = node.module || 'HtmlElement';
        ...
        if (moduleName === 'Component') {
            const cs = [{ name: 'componentName', value: node.componentName }];
    ...
    function dbeApplyHtmlTree(rootId, tree, done) {
        ...
        const origIdsInOrder = [];
        (function collect(id) {
            origIdsInOrder.push(id);
            (idx[id] || []).forEach(collect);
        })(rootId);
        const deleted = {};
        origIdsInOrder.forEach((id) => { if (!kept[id]) { deleted[id] = true; } });
        // Only the top-most removed nodes need driving — children go with them.
        const topDeleted = origIdsInOrder.filter((id) => {
            return deleted[id] && !deleted[(mods[id] && mods[id].parent) || ''];
        });

        const counts = { kept: 0, added: 0, removed: Object.keys(deleted).length };
        const wasBusy = dbeUndoBusy;
        dbeUndoBusy = true;
        ...
        function build() {
            ...
            function place(node, parentId, index) {
                ...
                if (node.existingId) {
                    const live = sf2.storeGet('modules') || {};
                    const m = live[node.existingId] && JSON.parse(JSON.stringify(live[node.existingId]));
                    if (m) {
                        ...
                        const keep = (m.settings || []).filter((x) => {
                ...
                (node.children || []).forEach((c, i) => { place(c, id, i); });
            }
            ...
            (function removeNext(i) {
                ...
                driveContextMenuItem(topDeleted[i], 'Remove', () => {
                    setTimeout(() => { removeNext(i + 1); }, 150);
                });
            })(0);
        }
    ...
    function dbeMakeCodeEditor(opts) {
        opts = opts || {};
        const api = window.Builderius && window.Builderius.API && window.Builderius.API.monaco;
        if (api && api.editor && typeof api.editor.create === 'function') {
            ...
            if (ed) {
                ...
                let autoCloseChangeListener = null;
                let autoCloseTimer = null;
                let classCompletionProvider = null;
                let classCompletionItems = null;
                if (opts.onEscape && typeof ed.onKeyDown === 'function') {
                    escapeKeyListener = ed.onKeyDown((event) => {
                ...
                if (typeof ed.onDidChangeModelContent === 'function') {
                    autoCloseChangeListener = ed.onDidChangeModelContent((event) => {
                        const changes = event && event.changes;
                        if (!changes || changes.length !== 1 || changes[0].text !== '>' || changes[0].rangeLength) { return; }
                        const cursorOffset = changes[0].rangeOffset + 1;
                        dbeClearOwnedTimeout(DBE_EDITING_OWNER, autoCloseTimer);
                        autoCloseTimer = dbeSetOwnedTimeout(DBE_EDITING_OWNER, () => {
                            autoCloseTimer = null;
                            const model = ed.getModel();
                            const selection = ed.getSelection();
                            if (!model || !selection ||
                                (typeof selection.isEmpty === 'function' && !selection.isEmpty())) { return; }
                            const liveCursorOffset = model.getOffsetAt({
                                lineNumber: selection.startLineNumber,
                                column: selection.startColumn
                            });
                            if (liveCursorOffset !== cursorOffset) { return; }
                            const value = model.getValue();
                            const tagStart = cursorOffset - 1;
                            if (value.charAt(tagStart) !== '>') { return; }
                            const withoutTypedBracket = value.slice(0, tagStart) + value.slice(cursorOffset);
                            const tag = dbeHtmlAutoCloseTag(withoutTypedBracket, tagStart, tagStart);
                            if (!tag) { return; }
                ...
                if (api.languages && typeof api.languages.registerCompletionItemProvider === 'function') {
                    classCompletionProvider = api.languages.registerCompletionItemProvider('html', {
                        ...
                        provideCompletionItems (model, position) {
                            ...
                            const context = dbeHtmlCompletionContext(model, position);
                ...
                function decorateMarkers() {
                    try {
                        const model = ed.getModel();
                        if (!model) { return; }
                        const matches = model.findMatches(' ?data-dbe-id="[^"]*"', false, true, false, null, false);
                        markerDecos = ed.deltaDecorations(markerDecos, matches.map((mm) => {
                ...
                function selectOffsets(start, end) {
                    try {
                        ...
                        const b = model.getPositionAt(end == null ? start : end);
                        ed.setSelection({
                ...
                function replaceOffsets(edits, selectStart, selectEnd) {
                    const model = ed.getModel();
                    if (!model) { return; }
                    ed.executeEdits('dbe-html-authoring', edits.map((edit) => {
                        const a = model.getPositionAt(edit.start);
                        const b = model.getPositionAt(edit.end);
                        return {
                            range: {
                                startLineNumber: a.lineNumber,
                                startColumn: a.column,
                ...
                function setDiagnostic(issue) {
                    ...
                    if (issue) {
                        ...
                        const end = model.getPositionAt(issue.offset + Math.max(1, issue.length || 1));
                        markers.push({
                ...
                return {
                    ...
                    setValue (v) { if (ed.getValue() !== v) { ed.setValue(v); decorateMarkers(); } },
                    focus () { try { ed.focus(); } catch (e) {} },
                    cursorStart () { try { ed.setPosition({ lineNumber: 1, column: 1 }); } catch (e) {} },
                    getSelection: selectionOffsets,
                    selectedText () {
                        const range = selectionOffsets();
                        return ed.getValue().slice(range.start, range.end);
                    },
                    replaceSelection (text, selectInserted) {
                        const range = selectionOffsets();
                        replaceOffsets([{ start: range.start, end: range.end, text }],
        ...
        function autoCloseTextareaTag(event) {
            ...
            const tag = dbeHtmlAutoCloseTag(ta.value, start, end);
            if (!tag) { return; }
            event.preventDefault();
        ...
        function textareaReplaceRanges(edits, selectStart, selectEnd) {
            edits.slice().sort((a, b) => { return b.start - a.start; }).forEach((edit) => {
        ...
        return {
            ...
            focus () { ta.focus(); },
            cursorStart () { try { ta.setSelectionRange(0, 0); } catch (e) {} },
            getSelection: textareaSelection,
            selectedText () {
                const range = textareaSelection();
                return ta.value.slice(range.start, range.end);
            },
            replaceSelection (text, selectInserted) {
                const range = textareaSelection();
                textareaReplaceRanges([{ start: range.start, end: range.end, text }],
                    selectInserted ? range.start : range.start + text.length,
                    selectInserted ? range.start + text.length : undefined);
            },
            ...
            layout () {},
            dispose () { ta.removeEventListener('keydown', autoCloseTextareaTag); }
        };
    }
    ...
    function dbeParsedNodeHtml(node, depth) {
        ...
        if ((node.module || 'HtmlElement') === 'Component') {
            let component = pad + '<dbe-component name="' + dbeHtmlEscapeAttr(node.componentName) + '"';
        ...
        if ((node.module || 'HtmlElement') !== 'HtmlElement' &&
            !(node.attrs || []).some((attr) => { return attr.name === 'data-dbe-module'; })) {
        ...
        (node.children || []).forEach((child) => { lines.push(dbeParsedNodeHtml(child, (depth || 0) + 1)); });
        lines.push(pad + '</' + tag + '>');
        return lines.join('\n');
    }
    ...
    function dbeHtmlAuthoringToolbar(editor, opts) {
        ...
        tool(dbeT('htmlFormat', 'Format'), () => {
            const range = editor.getSelection();
            const selected = range.end > range.start;
            const source = selected ? editor.selectedText() : editor.getValue();
            const analysis = dbeHtmlAnalyse(source);
        ...
        tool(dbeT('htmlRenameTag', 'Rename tag…'), () => {
            const value = editor.getValue();
            const analysis = dbeHtmlAnalyse(value);
            if (analysis.error) { announce(analysis.error.message, true); return; }
            const caret = editor.getSelection().start;
            let tokenIndex = -1;
            analysis.tokens.forEach((token, index) => {
                if (caret >= token.start && caret <= token.end) { tokenIndex = index; }
            });
            const pairIndex = tokenIndex === -1 ? null : analysis.pairs[tokenIndex];
            if (pairIndex == null) {
                announce(dbeT('htmlRenameNeedsPair', 'Place the caret in a paired opening or closing tag'), true);
                return;
            }
            const token = analysis.tokens[tokenIndex];
            ...
            const entered = window.prompt(dbeT('htmlRenamePrompt', 'Rename the matching tags to:'), token.rawName);
            if (entered == null) { return; }
            const name = dbeCleanTagInput(entered);
            if (!name || DBE_HTML_VOID[name]) {
                announce(dbeT('htmlRenameInvalid', 'Enter a non-void HTML tag name'), true);
                return;
            }
            const pair = analysis.tokens[pairIndex];
        ...
        if (opts.collection !== false) {
            tool(dbeT('htmlCollectionJson', 'Collection + JSON'), () => {
                ...
                const collapsed = dbeCollapseRepeats(parsed.roots, true);
                const collapsedRoot = collapsed.roots[0];
                ...
                editor.replaceSelection(collapsed.roots.map((root) => {
        ...
        if (opts.component) {
            componentButton = tool(dbeT('htmlCreateComponent', 'Create component after Apply'), () => {
                if (componentTarget) {
                ...
                const root = parsed.roots.length === 1 ? parsed.roots[0] : null;
        ...
        const jump = tool(dbeT('htmlJumpIssue', 'Go to issue'), () => {
    ...
    function dbeEditingDialogFocusReturn(preferred) {
        let target = preferred && preferred.isConnected ? preferred : null;
        if (!target) {
    ...
    function dbeBindEditingDialogEscape(dlg, surface) {
        ...
        (surface || dlg).addEventListener('keydown', (e) => {
    ...
    function dbeRemovePriorHtmlDialog() {
        const old = document.querySelector('dialog.dbe-html');
        if (!old) { return; }
        if (old.open) { try { old.close(); } catch (e) {} }
        if (old.isConnected) { old.remove(); }
    }
    ...
    function openEditHtmlDialog(rootId) {
        ...
        const editor = dbeMakeCodeEditor({
        ...
        let componentTargetId = null;
        const authoring = dbeHtmlAuthoringToolbar(editor, {
            origIds,
            rootId,
            component: true,
            setComponentTarget (id) { componentTargetId = id; },
        ...
        const undoWarning = dbeHtmlUndoWarning('dbe-edit-html-undo-warning',
        ...
        let reviewedHtml = null;
        apply.addEventListener('click', () => {
            let parsed;
            const currentHtml = editor.getValue();
            const analysis = dbeHtmlAnalyse(currentHtml);
            ...
            setTimeout(() => {
                dbeApplyHtmlTree(rootId, parsed.tree, (counts) => {
                    dbeHtmlBusy = false;
                    ...
                    let msg = dbeFmt(dbeT('htmlApplied', 'HTML applied: %1$s updated, %2$s added, %3$s removed'),
                        counts.kept, counts.added, counts.removed);
                    if (parsed.stripped.length) {
                        const uniq = parsed.stripped.filter((v, i, a) => { return a.indexOf(v) === i; });
                    ...
                    if (componentTargetId) {
                        dbeSetOwnedTimeout(DBE_EDITING_OWNER, () => {
                            ...
                            driveContextMenuItem(componentTargetId, 'Create Component', (ok) => {
                                if (!ok) {
        ...
        function updatePreview() {
            let parsed;
            const analysis = dbeHtmlAnalyse(editor.getValue());
            authoring.setIssue(analysis.error);
            if (analysis.error) {
                status.textContent = analysis.error.message;
                status.classList.add('dbe-html__status--warn');
            ...
            const c = dbePreviewCounts(parsed.tree, origIds, rootId);
            let msg = dbeFmt(dbeT('editHtmlWillApply', 'Will apply: %1$s updated, %2$s added, %3$s removed'),
    ...
    function dbeInsertParsedNode(sf, node, parentId, index) {
        ...
        let count = 1;
        node.children.forEach((c, i) => { count += dbeInsertParsedNode(sf, c, mod.id, i).count; });
        return { id: mod.id, count };
    }
    ...
    function dbeImportSlot(targetId) {
        ...
        const mods = sf.storeGet('modules') || {};
        if (!mods[targetId]) { return null; }
        const tag = String(dbeSettingVal(mods[targetId], 'tag') || '').toLowerCase();
        const indexes = sf.storeGet('indexes') || {};
        if (!DBE_HTML_VOID[tag]) {
            return { parentId: targetId, index: (indexes[targetId] || []).length, into: true };
        }
        const parentId = mods[targetId].parent || '';
        const sibs = [].concat(indexes[parentId || 'root'] || []);
        return { parentId, index: sibs.indexOf(targetId) + 1, into: false };
    }
    ...
    function dbeRepeatCandidate(n) {
        const moduleName = n.module || 'HtmlElement';
        const declared = moduleName === 'Collection' || moduleName === 'SubCollection';
        if (!declared && moduleName !== 'HtmlElement') { return false; }
        const kids = n.children;
        if (kids.length < (declared ? 1 : 2)) { return false; }
        if (!kids.every((c) => { return (c.module || 'HtmlElement') === 'HtmlElement'; })) { return false; }
        const sig = dbeNodeSignature(kids[0]);
        if (!kids.every((c) => { return dbeNodeSignature(c) === sig; })) { return false; }
    ...
    function dbeExtractRepeatData(copies, stats) {
        ...
        function walk(nodes) {
            ...
            if ((first.module || 'HtmlElement') === 'Component') {
                if (differs(nodes.map((n) => { return JSON.stringify(n.props || []); }))) { stats.opaque += 1; }
                return;
            }
            if (first.svg != null && first.svg !== '') {
                if (differs(nodes.map((n) => { return n.svg || ''; }))) { stats.opaque += 1; }
                return;
            }
            const contents = nodes.map((n) => { return n.content || ''; });
            if (differs(contents)) {
                const f = dbeFieldSlug(first, '', used);
                items.forEach((it, i) => { it[f] = contents[i]; });
                first.content = '{{' + f + '}}';
            }
            (first.attrs || []).forEach((a) => {
                const vals = nodes.map((n) => { return attrVal(n, a.name); });
                if (differs(vals)) {
                    const fa = dbeFieldSlug(first, a.name, used);
                    items.forEach((it, i) => { it[fa] = vals[i]; });
                    a.value = '{{' + fa + '}}';
                }
            });
            first.children.forEach((c, ci) => {
                walk(nodes.map((n) => { return n.children[ci]; }));
            });
        }
        walk(copies);
        return items;
    ...
    function dbeCollapseRepeats(roots, wire) {
        ...
        function walk(n) {
            ...
            if (dbeRepeatCandidate(n)) {
                const hasCtx = (n.attrs || []).some((a) => { return a.name === 'data-b-context'; });
    ...
    function dbePreviewLines(roots) {
        ...
        function walk(n, d) {
            ...
            const ctx = (n.attrs || []).filter((a) => { return a.name === 'data-b-context'; })[0];
            if (ctx) {
                let count = 0;
                try {
                    const arr = JSON.parse(ctx.value);
            ...
            if (n.content) {
                const t = n.content.length > 34 ? n.content.slice(0, 34) + '…' : n.content;
                line += ' “' + t + '”';
            }
            lines.push(line);
    ...
    function dbePreviewCounts(root, origIds, rootId) {
        ...
        let removed = 0;
        Object.keys(origIds).forEach((id) => { if (!preserved[id]) { removed += 1; } });
    ...
    function openImportHtmlDialog(targetId) {
        ...
        const title = document.createElement('h2');
        title.className = 'dbe-html__title';
        const slot = dbeImportSlot(targetId);
        ...
        const undoWarning = dbeHtmlUndoWarning('dbe-import-html-undo-warning',
        ...
        function refreshPreview() {
            ...
            const analysis = dbeHtmlAnalyse(html);
            authoring.setIssue(analysis.error);
            if (analysis.error) {
                preview.textContent = '';
                optionRow.style.display = 'none';
            ...
            const repeats = dbeFindRepeats(p.roots).length;
            ...
            const collapsing = !!(repeats && collapseCheck.checked);
            wireRow.style.display = collapsing ? '' : 'none';
            const collapsed = collapsing ? dbeCollapseRepeats(p.roots, wireCheck.checked) : null;
            const roots = collapsing ? collapsed.roots : p.roots;
            parsed = { roots, stripped: p.stripped };
            preview.textContent = dbePreviewLines(roots).join('\n');
            const total = dbePreviewLines(roots).length;
            let note = dbeFmt(dbeTn(total,
                'importCountOne', '%s element will be created.',
                'importCountMany', '%s elements will be created.'), total);
            if (p.stripped.length) {
                const uniq = p.stripped.filter((v, i, a) => { return a.indexOf(v) === i; });
        ...
        insert.addEventListener('click', () => {
            ...
            const liveSlot = dbeImportSlot(targetId); // re-read: the tree may have moved on
            ...
            try {
                const sf = store();
                roots.forEach((r, i) => {
                    const res = dbeInsertParsedNode(sf, r, liveSlot.parentId, liveSlot.index + i);
                    count += res.count;
    ...
    const DBE_TAG_MODULES = { HtmlElement: 1, Collection: 1, SubCollection: 1 };
    const DBE_TAG_CHOICES = [
    ...
    function dbeChangeTagEligible(id) {
        const m = (modules() || {})[id];
        const tag = m && DBE_TAG_MODULES[m.name]
    ...
    function dbeCleanTagInput(v) {
        const t = String(v || '').trim().toLowerCase().replace(/^</, '').replace(/>$/, '').trim();
    ...
    function dbeChangeTag(id, newTag) {
        ...
        const m = mods[id];
        if (!m) { return; }
        const oldTag = String(dbeSettingVal(m, 'tag') || '').toLowerCase();
        if (!oldTag || oldTag === newTag) { return; }
        const labelWasDefault = (m.label || '').toLowerCase() === oldTag;
        dbeUpdateModuleSettings(id, (settings) => {
            const t = settings.filter((s) => { return s.name === 'tag'; })[0];
    ...
    function dbeRestoreOp(rec, cb) {
        ...
        const paste = function (menuRowId) {
            let prevClip = null;
            navigator.clipboard.readText()
                .then((t) => { prevClip = t; })
                .catch(() => {})
                .then(() => { return navigator.clipboard.writeText(payload); })
                .then(() => {
                    driveContextMenuItem(menuRowId, 'Paste', (ok) => {
                        ...
                        waitFor(() => {
                            ...
                            if (prevClip !== null) { navigator.clipboard.writeText(prevClip).catch(() => {}); }
                ...
                .catch(() => { cb(null, dbeT('undoFailedClipboard', 'Undo failed: clipboard blocked')); });
        };
        if (rec.parentId) {
            ...
            (function selectParent() {
                const row = document.querySelector('.uniRightPanel .uni-tree-node-' + rec.parentId);
                if (row) { clickSeq(row); }
                waitFor(() => { return activeId() === rec.parentId || null; }, (ok) => {
    ...
    function dbeRunHistory(from, to, emptyKey, emptyDef, successAction) {
        ...
        if (rec.op === 'settings') {
            ...
            dbeUndoBusy = true;
            try {
                dbeUpdateModuleSettings(rec.id, (settings) => {
                    const restored = JSON.parse(JSON.stringify(rec.settings || []));
                    settings.length = 0;
                    restored.forEach((setting) => { settings.push(setting); });
            ...
            const current = dbeMoveLocation(rec.id);
            ...
            const subtree = dbeCollectSubtree(mods, rec.id);
            const placement = dbeHistoryPlacement(mods, store().storeGet('indexes') || {}, rec.id);
            const parentId = placement.parentId;
            dbeUndoBusy = true;
            driveContextMenuItem(rec.id, 'Remove', (ok) => {
                setTimeout(() => {
                    dbeUndoBusy = false;
                    if (!ok) { from.push(rec); undoToast(dbeT('undoFailedRemove', 'Undo failed: could not remove the element')); return; }
                    to.push({
                        op: 'restore', id: rec.id, label: rec.label, parentId,
    ...
    function bindUndoKeys() {
        dbeBindOwnedEvent(DBE_EDITING_OWNER, document, 'history-key', 'keydown', (e) => {
    ...
    let saveBaseline = null;
    let saveBaselineSnapshot = null;
    let saveSettingsBaseline = null;
    let dbeSaveInitialisingUntil = 0;
    let dbeSaveState = '';
    let dbeSavePending = null;
    let dbeSaveTimer = null;
    let dbeSaveIgnoreDirtyUntil = 0;
    let dbeSaveLastStamp = 0;
    let dbeSaveSnapshotItem = null;
    let dbeSaveSnapshotSignature = null;
    ...
    function dbeSaveStamp() {
        try {
            const h = store().storeGet('history') || [];
    ...
    function dbeSaveableSnapshotSignature() {
        try {
            const history = store().storeGet('history') || [];
            const item = history.length ? history[history.length - 1] : null;
    ...
    function dbeHasUnsavedChanges(len, shouldSave, snapshotSignature) {
        ...
        const settingsDirty = saveSettingsBaseline === false && shouldSave;
        return len > saveBaseline || (Date.now() > dbeSaveIgnoreDirtyUntil && settingsDirty);
    }
    function dbeRenderSaveCue() {
        const cue = document.querySelector('.dbe-save-cue');
        if (!cue) { return; }
        const len = historyLen();
        if (len === null) { return; }
        const shouldSave = dbeShouldSave();
        const snapshotSignature = dbeSaveableSnapshotSignature();
        ...
        const dirty = dbeHasUnsavedChanges(len, shouldSave, snapshotSignature);
        dbePresenceDirtyChanged(dirty);
        if (dbeSaveState === 'saved' && dirty) { dbeSaveState = ''; }
        const state = dbeSaveState || (dirty ? 'dirty' : 'clean');
        const text = state === 'saving' ? dbeT('saving', 'Saving…')
        ...
        const shortText = state === 'saving' ? dbeT('saving', 'Saving…')
            : state === 'error' ? dbeT('saveFailedShort', 'Save failed')
                : state === 'dirty' ? dbeT('unsavedShort', 'Unsaved')
                    : dbeT('saveCleanShort', 'Saved');
        let full = cue.querySelector('.dbe-save-cue__full');
        let short = cue.querySelector('.dbe-save-cue__short');
        if (!full || !short) {
            cue.textContent = '';
        ...
        ['clean', 'dirty', 'saving', 'saved', 'error'].forEach((name) => {
    ...
    function dbeShowSavedState(stamp) {
        ...
        dbeSaveTimer = dbeSetOwnedTimeout(DBE_EDITING_OWNER, () => {
            dbeSaveTimer = null;
    ...
    function dbeFinishSave() {
        ...
        const pending = dbeSavePending;
        dbeSavePending = null;
        dbeClearOwnedTimeout(DBE_EDITING_OWNER, dbeSaveTimer);
        dbeSaveTimer = null;
        const confirmed = stamp >= pending.started && stamp !== pending.beforeStamp;
    ...
    function ensureSaveCue() {
        ...
        const save = dbeQuery('saveButton');
        if (!save) { return; }
        let cue = document.querySelector('.dbe-save-cue');
        if (!cue) {
    ...
    function bindSaveShortcut() {
        dbeBindOwnedEvent(DBE_EDITING_OWNER, document, 'save-shortcut', 'keydown', (e) => {
            if (e.repeat || (e.key || '').toLowerCase() !== 's') { return; }
            const mod = dbeIsMac ? (e.metaKey && !e.ctrlKey) : e.ctrlKey;
    ...
    let dbeAttrSeededRow = null; // the blank row we added, awaiting use or cleanup
    let dbeAttrSeededFor = null; // activeModule id we last seeded for (double-seed guard)
    let dbeAttrSeedOwned = false; // true only while our native Add click remains unsettled
    ...
    const ATTR_ROW_SEL = '.uniSettingHtmlAttribute_item, .uniSettingHtmlAttribute_itemEdit';
    ...
    function attrRowBlank(row) {
        const inputs = [].slice.call(row.querySelectorAll('input'));
        return inputs.length > 0 && inputs.every((i) => { return !(i.value || '').trim(); });
    }
    function attrRemoveRow(row) {
        const btn = row.querySelector('.uniSettingHtmlAttribute_itemActions button') || row.querySelector('button');
        if (btn) { clickSeq(btn); }
    }
    ...
    function dbeRemoveBlankAttrs(id) {
        ...
        const mod = mods[id];
        if (!mod) { return; }
        const ha = (mod.settings || []).filter((s) => { return s.name === 'htmlAttribute'; })[0];
        if (!ha || !Array.isArray(ha.value)) { return; }
        if (!ha.value.some((a) => { return !(a && (a.name || '').trim()); })) { return; }
        dbeUpdateModuleSettings(id, (settings) => {
            const h = settings.filter((s) => { return s.name === 'htmlAttribute'; })[0];
            if (h && Array.isArray(h.value)) {
                h.value = h.value.filter((a) => { return a && (a.name || '').trim(); });
            }
        });
    }
    ...
    function ensureAttrDatalist() {
        ...
        ['id', 'role', 'title', 'tabindex', 'aria-label', 'aria-labelledby', 'aria-describedby',
         'aria-hidden', 'aria-live', 'data-', 'lang', 'dir', 'hidden'].forEach((n) => {
    ...
    function bindAttrQuickPick(list) {
        ensureAttrDatalist();
        attrRows(list).forEach((row) => {
            const nameInput = attrNameInput(row);
            ...
            [].slice.call(row.querySelectorAll('input')).forEach((inp) => {
    ...
    function bindAttrAutoClean() {
        ...
        dbeBindOwnedEvent(DBE_EDITING_OWNER, document, 'attribute-seed-clean', 'focusout', () => {
            if (!dbeAttrSeededRow) { return; }
            dbeSetOwnedTimeout(DBE_EDITING_OWNER, () => {
                const r = dbeAttrSeededRow;
                if (!r) { return; }
    ...
    function ensureBlankAttrRow() {
        ...
        const rows = attrRows(list);
        if (rows.length > 0) { dbeAttrSeededFor = mid; return; } // user already has rows
        const addBtn = (list.parentNode && list.parentNode.querySelector('button.uniSettingHtmlAttribute_addNewBtn')) ||
    ...
    let dbeCondSeededFor = null;   // element id the blank card was seeded for
    let dbeCondSeededCard = null;  // the seeded card node, until settled
    let dbeCondViewWas = false;    // view presence last tick (seed on opening edge)
    let dbeCondSeedEngaged = false; // the user interacted with the seeded card
    let dbeCondSeedOwned = false;  // true only for the placeholder DBE added
    ...
    function condA11y(view) {
        [].slice.call(view.querySelectorAll('.uniElementConditions_itemActions .uniIconButton')).forEach((b) => {
        ...
        [].slice.call(view.querySelectorAll('.uniElementConditions_itemEdit, .uniElementConditions_item')).forEach((item) => {
            // First <select> in a card is the comparison operator; any later
            // one picks the value.
            [].slice.call(item.querySelectorAll('.uniElementConditions_itemFields select')).forEach((s, i) => {
                const want = i === 0 ? dbeT('condOperator', 'Comparison') : dbeT('condValue', 'Value');
        ...
        [].slice.call(view.querySelectorAll('.uniElementConditions_itemFields input:not([type="hidden"])')).forEach((inp) => {
        ...
        [].slice.call(view.querySelectorAll('.uniSystemSelect__freeInputBtn, .builderiusSelectFreeInputToggle')).forEach((b) => {
            if (!b.getAttribute('aria-label')) {
                dbeRememberOwnedAttributes(DBE_EDITING_OWNER, b, ['aria-label']);
    ...
    function condSeedClean() {
        ...
        if (id && condBlankOnly(id)) {
            const x = card.querySelector('.uniElementConditions_itemActions .uniIconButton');
            if (x) { clickSeq(x); }
        }
    ...
    function bindCondAutoClean() {
        dbeBindOwnedEvent(DBE_EDITING_OWNER, document, 'condition-seed-clean', 'focusout', () => {
    ...
    function condSeedBlank(view) {
        ...
        const add = view.querySelector('.uniElementConditions_addNewBtn');
        if (!add) { return; }
        dbeCondSeededFor = id;
        ...
        waitFor(() => {
            ...
            const left = document.querySelector('.uniLeftPanelOuter') || document.querySelector('.uniLeftPanel');
            if (left && left.contains(document.activeElement)) {
                const typeSel = card.querySelector('.uniSystemSelect');
    ...
    function condCues() {
        ...
        const icon = document.querySelector('.uniIconConditionsMode');
        if (icon) {
            dbeRememberOwnedAttributes(DBE_EDITING_OWNER, icon, ['aria-label', 'data-dbe-tip']);
            const id = activeId();
            const n = id ? condCount(id) : 0;
            const has = n > 0;
        ...
        [].slice.call(document.querySelectorAll('.uniRightPanel button.uniModTree__item')).forEach((btn) => {
            const m = btn.className.toString().match(/uni-tree-node-(\w+)/);
            const has2 = !!(m && withCond[m[1]]);
            const descriptionId = m ? 'dbe-cond-desc-' + m[1] : '';
            const dot = btn.querySelector('.dbe-cond-dot');
            if (has2) {
                if (!dot) {
                ...
                const descriptions = (btn.getAttribute('aria-describedby') || '').split(/\s+/).filter(Boolean);
                ...
                dot.remove();
                const sr2 = btn.querySelector('.dbe-cond-sr');
                if (sr2) { sr2.remove(); }
                const remaining = (btn.getAttribute('aria-describedby') || '').split(/\s+/).filter((id) => {
    ...
    function ensureConditionHelpers() {
        bindCondAutoClean();
        condCues();
        const view = document.querySelector('.uniLeftPanel .uniElementConditions') || document.querySelector('.uniElementConditions');
        if (!view) {
        ...
        const seeded = condSeedResolve();
        if (seeded) {
            const mark = !dbeCondSeedEngaged;
    ...
    let dbePropStatus = null;

    const PROP_ROW_SEL = '.uniSettingComponentTmplProperties_item, .uniSettingComponentTmplProperties_itemEdit';
    ...
    function propList() {
        const lists = [].slice.call(document.querySelectorAll('ul.uniSettingComponentTmplProperties_list'));
    ...
    function propItems(list) {
        return [].slice.call(list.children).filter((li) => {
    ...
    function propLabel(li) {
        const n = li.querySelector('.uniSettingComponentTmplProperties_itemName');
    ...
    function entitySettings() {
        try {
            const ent = store().storeGet('entity');
            if (ent && ent.settings) { return ent.settings; }
        } catch (e) {}
        return null;
    }
    function entitySetting(name) {
        const s = entitySettings();
        if (!s) { return undefined; }
        if (Array.isArray(s)) {
            const hit = s.filter((e) => { return e && e.name === name; })[0];
    ...
    function readComponentProps() {
        const v = entitySetting('componentTmplProperties');
        if (Array.isArray(v)) { return v; }
        const list = propList();
        if (!list) { return null; }
        try {
            const fk = Object.keys(list).find((k) => { return k.indexOf('__reactFiber$') === 0; });
            let f = fk && list[fk];
            for (let i = 0; i < 12 && f; i++, f = f.return) {
                ...
                for (const key in p) {
                    const val = p[key];
                    if (Array.isArray(val) && val.length && val.every((it) => {
    ...
    function writeComponentProps(arr) {
        try {
            const payload = [{ name: 'componentTmplProperties', setting: arr }];
            const dv = entitySetting('dataVars');
    ...
    function propPersistOrder(list) {
        const rows = propItems(list);
        const perm = rows.map((li) => { return parseInt(li.getAttribute('data-dbe-prop-idx'), 10); });
        if (perm.some((v) => { return isNaN(v); })) { return; }
        if (perm.every((v, i) => { return v === i; })) { return; } // unchanged
        const old = readComponentProps();
        if (!old) { propAnnounce(dbeT('propSaveFailed', 'Order changed on screen, but it could not be saved to the component')); return; }
        const next = perm.map((idx) => { return old[idx]; });
        if (next.some((x) => { return x === undefined; }) || next.length !== old.length) { return; } // stale — don't corrupt
    ...
    function setPropMode(list, onMode) {
        const container = list.closest('.uniSettingComponentTmplProperties') || list.parentNode;
        const btn = container && container.querySelector('.dbe-prop-reorder-btn');
        ...
        if (onMode) {
            ...
            propItems(list).forEach((li) => {
                li.removeAttribute('tabindex');
                const nm = li.querySelector('.uniSettingComponentTmplProperties_itemName[data-dbe-prop-label]');
    ...
    function bindPropDrag(list) {
        let drag = null;
        dbeBindOwnedEvent(DBE_EDITING_OWNER, list, 'properties-reorder-click', 'click', (ev) => {
            if (!list.classList.contains('dbe-prop-reordering')) { return; }
            if (ev.target.closest && ev.target.closest('.dbe-prop-reorder')) { return; }
            ev.preventDefault();
            ev.stopPropagation();
        }, true);
        dbeBindOwnedEvent(DBE_EDITING_OWNER, list, 'properties-reorder-pointerdown', 'pointerdown', (ev) => {
            if (!list.classList.contains('dbe-prop-reordering')) { return; }
            const li = ev.target.closest && ev.target.closest(PROP_ROW_SEL);
            if (!li) { return; }
            ev.preventDefault();
            ev.stopPropagation();
            drag = { li };
        ...
        let propRaf = 0, propY = 0;
        dbeBindOwnedEvent(DBE_EDITING_OWNER, list, 'properties-reorder-pointermove', 'pointermove', (ev) => {
            if (!drag) { return; }
            propY = ev.clientY;
            if (propRaf) { return; }
            propRaf = dbeSetOwnedFrame(DBE_EDITING_OWNER, () => {
                propRaf = 0;
                if (!drag) { return; } // drag ended before the frame
                const items = propItems(list).filter((it) => { return it !== drag.li; });
                for (let i = 0; i < items.length; i++) {
                    const r = items[i].getBoundingClientRect();
                    if (propY >= r.top && propY <= r.bottom) {
                        const before = propY < r.top + r.height / 2;
        ...
        dbeBindOwnedEvent(DBE_EDITING_OWNER, list, 'properties-reorder-keys', 'keydown', (ev) => {
            if (!list.classList.contains('dbe-prop-reordering')) { return; }
            if (ev.key === 'Escape') {
                ev.preventDefault();
                ev.stopPropagation();
                ...
                const container = list.closest('.uniSettingComponentTmplProperties') || list.parentNode;
                const b = container && container.querySelector('.dbe-prop-reorder-btn');
                if (b) { b.focus(); }
                return;
            }
    ...
    function ensurePropertiesReorder() {
        ...
        const addBtn = container.querySelector('.uniSettingComponentTmplProperties_addNewBtn');
    ...
    function dbeResetEditingHelpers() {
        ...
        dbeAttrSeedOwned = false;
        const attrNames = document.getElementById('dbe-attr-names');
        if (attrNames) { attrNames.remove(); }

        const seededCondition = condSeedResolve();
        if (seededCondition) {
            ...
            if (dbeCondSeedOwned && !dbeCondSeedEngaged && dbeCondSeededFor && condBlankOnly(dbeCondSeededFor)) {
                const remove = seededCondition.querySelector('.uniElementConditions_itemActions .uniIconButton');
        ...
        dbeCondSeedEngaged = false;
        dbeCondSeedOwned = false;
        document.querySelectorAll('.dbe-cond-dot, .dbe-cond-sr').forEach((node) => { node.remove(); });
        document.querySelectorAll('.dbe-has-cond').forEach((node) => { node.classList.remove('dbe-has-cond'); });

        document.querySelectorAll('.dbe-prop-reorder').forEach((node) => { node.remove(); });
        document.querySelectorAll('.dbe-prop-reordering').forEach((list) => {
            list.classList.remove('dbe-prop-reordering');
            propItems(list).forEach((item) => {
                item.classList.remove('dbe-prop-dragging');
                item.removeAttribute('data-dbe-prop-idx');
                item.removeAttribute('tabindex');
                const label = item.querySelector('[data-dbe-prop-label]');
    ...
    function bindDblclickRename() {
        dbeBindOwnedEvent(DBE_EDITING_OWNER, document, 'double-click-rename', 'dblclick', (e) => {
    ...
    const DBE_EDITING_OWNER = 'editing';
    let dbeEditingControllerActive = false;
    const NEED_EDITING = on('undo_delete') || on('image_defaults') || on('inline_rename') ||
    ...
    function destroyEditing() {
        ...
        dbeRemovePriorHtmlDialog();
        dbeRemovePriorBemDialog();
        const toast = document.querySelector('.dbe-undo-toast');
    ...
    dbeControllers.register(DBE_EDITING_OWNER, {
        ...
        host.setEditingApi(Object.freeze({
            storeMoveModule, undoToast, renameActive,
            moveSibling, indent: indentElement, outdent: outdentElement, selectParent: selectParentOf,
            startRename, defaultLabel: defaultLabelFor, commitRename, wrap, unwrap,
            bemClassable, openAutoBem: openAutoBemDialog, indentTarget: dbeIndentTarget, canOutdent: dbeCanOutdent,
            ...
            moveLocation: dbeMoveLocation, moduleClasses,
